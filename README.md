...
: `pip install nforge_migration` 명령어를 이용하지 않고, 본 프로젝트의 소스를 이용해서 직접 설치할 수도 있습니다. 
  ...
  ```
  $ git clone https://oss.navercorp.com/devcenter/open-project-migration.git
...
- Github로 마이그레이션을 위해서는 1) github 저장소 생성 2) 해당 저장소에 위키 생성 3) 해당 저장소에 대한 접근토큰값(Personal Access Token)이 있어야 합니다. 
...
* 마이그레이션 사전 준비
    ...
    4. [이곳을 참고하셔서](https://help.github.com/articles/creating-an-access-token-for-command-line-use/) `Personal Access Token`을 생성하신 후 프로젝트 디렉토리에 `data`라는 디렉토리를 만드시고 `github_ACCESS_TOKEN` 이라는 파일에 토큰을 넣어주세요. (파일을 만들지 않으시면 프로그램에서 토큰을 물어보는데 이때 토큰을 입력해주세요)

* 마이그레이션 모듈 실행
   1단계)  ghm 명령어를 실행하면 아래와 같이 옵션을 입력하는 프롬프트가 나오며, Public[y/n]:라는 물음에 대해 ‘y’로 입력하면 됩니다.
    ...
    ```
  2단계) `Nforge/open_project` 에 있는 프로젝트들 중 하나를 골라 마이그레이션을 수행하게 됩니다. 출력되는 번호들 중 하나를 입력해주세요. 없는 번호를 고르시면 프로그램이 종료됩니다.
...
* 마이그레이션 사전 준비
    1. [GitHub](https://github.com) 계정이 없는 분은 github 회원가입을 해주세요. **[참고](https://help.github.com/articles/signing-up-for-a-new-github-account/)**
     ...
     7. [이곳을 참고하셔서](https://help.github.com/articles/creating-an-access-token-for-command-line-use/) `Personal Access Token`을 생성하신 후 프로젝트 디렉토리에 `data`라는 디렉토리를 만드시고 `github_ACCESS_TOKEN` 이라는 파일에 토큰을 넣어주세요. (파일을 만들지 않으시면 프로그램에서 토큰을 물어보는데 이때 토큰을 입력해주세요)
